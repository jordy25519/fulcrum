...
library Buffer {
    ...
    /**
     * @dev Appends a bytes16 to the buffer. Resizes if doing so would exceed
     *      the capacity of the buffer.
     * @param buf The buffer to append to.
     * @param data The data to append.
     * @return The original buffer, for chhaining.
     */
    function appendBytes16(buffer memory buf, bytes16 data) internal pure returns (buffer memory) {
        return append(buf, bytes32(data), 16);
    }
